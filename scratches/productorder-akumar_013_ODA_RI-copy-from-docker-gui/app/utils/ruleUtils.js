...
async function validateSpecHref(payload) {
  console.log("validateSpecHref input:", JSON.stringify(payload, null, 2)); // Add this

  const href = payload?.productOrderItem[0]?.product?.productSpecification?.href;
  ...
  try {
    ...
    if (response.status !== 200) {
      throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `Specification href '${href}' is invalid.`);
    } else {
      console.log(`Successfully validated productSpecification.href: ${href}`);
    }
  } catch (err) {
...
async function validateInventoryHref(payload) {
  console.log("validateInventoryHref input:", JSON.stringify(payload, null, 2)); // Add this

  const href = payload?.productOrderItem[0]?.product?.href;
...
module.exports = { validateRequest, validateSpecHref, validateInventoryHref };
