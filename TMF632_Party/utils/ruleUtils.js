...
async function validateRelatedPartyHref(payload) {
  ...
  const party = payload?.relatedParty?.[0];
  let href = party?.href;
  ...
  if (!href) {
    ...
    */
    try {
        ...
        console.error("Failed rewriting relatedParty href:", err);
        console.log(`Rewriting relatedParty href:\n  from: ${href}\n    to: ${rewritten}`);
    }
    // Validate rewritten (or original) href
    try {
    ...
    if (response.status !== 200) {
      throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `RelatedParty href '${href}' is invalid.`);
    }
...
async function validateRelatedPartyHref_by_env_DEPENDENTAPI_PARTYROLE_SERVICE_NAME(payload) {
  ...
  */
    ...
    if (!serviceName) {
        console.warn("DEPENDENTAPI_PARTYROLE_SERVICE_NAME is not set!");
    } else {
        try {
            // Rewrite example: if  DEPENDENTAPI_PARTYROLE_SERVICE_NAME = nonsecuritypartyroleapi
            // Incoming: https://localhost/per-permission/tmf-api/partyRoleManagement/v4/partyRole/UUID
            // Desired: http://per-nonsecuritypartyroleapi/per-permission/tmf-api/partyRoleManagement/v4/partyRole/UUID
            const url = new URL(href);
            // Extract the first path segment "per-permission""
            const pathParts = url.pathname.split("/").filter(p => p.length > 0);
            const firstSegment = pathParts[0]; // e.g., "per-permission"
            // Extract prefix up to and including first '-' character
            const prefixMatch = firstSegment.match(/^[^-]+-/); // e.g., "per-"
            const prefix = prefixMatch ? prefixMatch[0] : "";
            const internalHost = `${prefix}${serviceName}`;
            const rewritten = `http://${internalHost}:8080${url.pathname}`;
            console.log(`Rewriting relatedParty href:\n  from: ${href}\n    to: ${rewritten}`);
            href = rewritten; // update the above href variable
        } catch (err) {
            console.error("Failed rewriting relatedParty href:", err);
        }
    }
